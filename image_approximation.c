 ...
 * from order matrix, to white otherwise
 *
 * image_info: pointer to IMAGE_INFO structure
...
void ordered_dithering(PIMAGE_INFO image_info, void *params) {
  ...
  double threshold = ORDERED_THRESHOLD(od_params->order_matrix,
...
/*
 * Function: init_order_matrix
 * ----------------------------
 * Generates order matrix for ordered dithering
 * D(2) = | 0 2 |
 *        | 3 1 |
 *
 ...
 * returns: pointer to matrix
 */
int **init_order_matrix(int size) {
  int i;
  int j;
  int prev_size;
  int **d_prev;
  ...
  prev_size = size/2;
  d_prev = init_order_matrix(prev_size);
...
/*
 * Function: deinit_order_matrix
 * ----------------------------
 * Deinits matrix generated by init_order_matrix
 *
 ...
 * returns: void
 */
void deinit_order_matrix(int **matrix, int size) {
...
int main(int argc, char *argv[]) {
  ...
  switch(atoi(argv[3])) {
    ...
    case ORDERED_DITHERING:
      ...
      od_params.order_matrix = init_order_matrix(od_params.matrix_size);
      seq_way(image_info, ordered_dithering, &od_params);
      deinit_order_matrix(od_params.order_matrix, od_params.matrix_size);
      break;
